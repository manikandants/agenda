...
Agenda.prototype.updateSchedule = function(when, name, data, cb) {
  var self = this;
...
Agenda.prototype.updateJob = function(job, cb) {
  var fn = cb,
      self = this;
  ...
  if (id) {
    ...
    // Try an upsert.
    this._db.findAndModify({name: props.name, data: data, type: 'single'}, {}, update, {upsert: true, new: true}, processDbResult);
  } else if (unique) {
    var query = {};
	query.data = job.attrs.unique;
    query.name = props.name;
    this._db.findAndModify(query, {}, update, {upsert: true, new: true}, processDbResult);
  } else {
    this._db.insert(props, processDbResult);
